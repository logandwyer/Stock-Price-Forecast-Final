...
# For fallback data source (Stooq)
import requests
...
def _fetch_stooq(symbol: str, start: pd.Timestamp) -> pd.DataFrame:
    """Fallback to Stooq CSV. Supports most US equities/ETFs (ticker.us)."""
    sym = symbol.strip().upper()
    # Stooq uses *.us for US listings; ^VIX is "vix" on Stooq but we don't need ^VIX here.
    stq = f"{sym.lower()}.us"
...
def fetch_prices(symbol: str, years: int) -> Tuple[pd.DataFrame, str]:
    ...
    # 1) yfinance download with small retries
    ...
    # 3) Stooq fallback (equities/ETFs)
    stq = _fetch_stooq(symbol, start=start)
    if not stq.empty:
        return stq, "Stooq"
...
with st.spinner("Downloading recent prices…"):
    df, source_label = fetch_prices(ticker, lookback_years)
...
c4.metric("Data source", source_label or "Unknown")
...
st.write(
    ...
    f"- In plain English: Based on recent behavior, the stock’s middle path reaches "
