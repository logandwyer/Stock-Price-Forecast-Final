# app.py
# Streamlit macro-informed stock forecaster with auto-install bootstrap for missing deps.
...
def _ensure_packages():
    ...
    if missing:
        # One-shot install; no loops.
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "--disable-pip-version-check", "--no-input", *missing])
        except Exception as e:
            # Allow app to continue; downstream code will show a helpful error if imports still fail.
            print("Package auto-install failed:", e)
...
# ---- (identical logic from previous app below) ----
import math
from dataclasses import dataclass
from typing import Dict, Tuple, Optional

def safe_pct_change(s, periods=1):
...
def fetch_all(ticker: str, lookback_years: int) -> pd.DataFrame:
    ...
    start = end - pd.DateOffset(years=int(lookback_years)+1)
...
forecast_table = pd.DataFrame([{"Horizon": k, "Mean Simulated Price": round(v, 2)} for k, v in forecast_points.items()])
...
def valuation_summary(S0: float, m6: float, m12: float, ann_vol: float) -> str:
    ...
    if r12 < -0.05 and ann_vol > 0.30:
        return "⚠️ Appears **overvalued / high risk**."
    if r12 > 0.05 and ann_vol <= 0.35:
        return "✅ Appears **undervalued / favorable**."
...
with tab1:
    ...
    with c1:
        st.write("**GBM Baseline (analytical)**"); st.dataframe(gbm_df, use_container_width=True)
    with c2:
        ...
        if PLOTLY_OK:
            ...
            for name, d in HORIZONS.items():
                ax.scatter([d], [mean_path[d]]); ax.text(d, mean_path[d], name, ha="center", va="bottom", fontsize=8)
...
def walk_forward_validation(close: pd.Series, feats: pd.DataFrame, regimes: pd.Series,
    ...
    for t in range(len(last_300) - horizon):
        ...
        mu_b = trail.mean(); sig_b = trail.std(ddof=1)
        ...
        if SKLEARN_OK and ml_model is not None and ml_on:
            ...
            if not xrow.empty:
                try:
                    pred21 = float(ml_model.predict(xrow)[0]); ml_drift = pred21 / horizon
                except Exception:
                    ml_drift = None
