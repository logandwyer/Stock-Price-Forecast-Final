...
def fetch_one(symbol: str, start: str, min_rows: int = 60, tries: int = 3) -> pd.DataFrame:
    """Robust single-symbol fetch with retries and a Ticker().history fallback."""
    # Normalize Yahoo symbol variants (e.g., BRK.B -> BRK-B)
    yahoo_sym = symbol.replace("/", "-").replace(".", "-").strip()

    for attempt in range(tries):
        try:
            df = yf.download(
                yahoo_sym, start=start, progress=False, auto_adjust=True,
    ...
    # Fallback: Ticker().history with period
    try:
...
def fetch_all(ticker: str, lookback_years: int) -> pd.DataFrame:
    ...
    for s in syms:
        ...
        if not part.empty:
            frames[s] = part
        time.sleep(0.1)  # be kind to Yahoo
...
def compute_indicators(df: pd.DataFrame):
    ...
    ind["VIX"] = df["^VIX"]; ind["SPY_mom63"] = df["SPY"].pct_change(63)
    ind["TNX"] = df["^TNX"]; ind["IRX"] = df["^IRX"]; ind["Spread"] = ind["TNX"] - ind["IRX"]
