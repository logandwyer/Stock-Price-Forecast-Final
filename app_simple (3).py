...
def regime_adjust_mu(mu: float, regime: str) -> float:
    """Smaller multipliers to avoid runaway projections."""
    mult = {"Bull": 1.10, "Neutral": 1.00, "Bear": 0.80}.get(regime, 1.00)
    return mu * mult

# ==============================
# CAPM-style drift blending
# ==============================
def compute_capm_mu(stock_close: pd.Series, spy_close: pd.Series, rf_ann: float = 0.03) -> float:
    """
    Estimate daily drift via a simple CAPM:
        mu_capm_daily = (rf + beta * ERP) / 252
    where ERP is clamped into [3%, 7%] to avoid extremes.
    """
    if spy_close is None or spy_close.dropna().empty or stock_close is None or stock_close.dropna().empty:
        # fallback to a modest baseline
        return (rf_ann + 0.05) / 252.0  # ~8% annual nominal

    r_s = np.log(stock_close).diff().dropna()
    r_m = np.log(spy_close).diff().dropna()
    # Align
    df = pd.concat([r_s, r_m], axis=1).dropna()
    if df.shape[0] < 60:
        return (rf_ann + 0.05) / 252.0

    rs = df.iloc[:,0].values
    rm = df.iloc[:,1].values
    var_m = float(np.var(rm, ddof=1))
    cov_sm = float(np.cov(rs, rm, ddof=1)[0,1])
    beta = cov_sm / var_m if var_m > 1e-12 else 1.0

    market_ann = 252.0 * float(np.mean(rm))
    erp = max(0.03, min(0.07, market_ann - rf_ann))  # clamp 3% to 7%
    mu_capm_daily = (rf_ann + beta * erp) / 252.0
    return mu_capm_daily

def blend_mu(mu_obs: float, mu_capm_daily: float, regime: str) -> float:
    """
    Blend observed drift with CAPM drift, with shrinkage.
    Heavier weight on CAPM; observed gets 30% weight.
    Regime multiplier applied after blending.
    """
    w_capm = 0.70
    mu_blend = w_capm * mu_capm_daily + (1.0 - w_capm) * mu_obs
    mu_adj = regime_adjust_mu(mu_blend, regime)
    # Clamp annualized drift to +/- 18%
    cap_ann = 0.18
    mu_adj = max(-cap_ann/252.0, min(cap_ann/252.0, mu_adj))
    return mu_adj

# ==============================
# GARCH-like volatility & realistic simulation
# ==============================
@dataclass
class GarchParams:
    omega: float = 1e-6
...
def simulate_path_realistic(
    ...
    n_paths: int,
    seed: Optional[int],
    mean_revert_kappa: float = 0.004,   # stronger than before to avoid runaway
    shock_prob: float = 0.02,           # 2% daily chance
    shock_sigma_mult: float = 2.5,      # ~2.5σ shock size
    gparams: GarchParams = GarchParams(),
) -> Tuple[np.ndarray, dict]:
    """Simulate prices with CAPM/regime-adjusted drift, GARCH-like vol, mean reversion, and shocks."""
    rng = np.random.default_rng(int(seed) if seed is not None else None)

    # Long-run anchor is today (pragmatic)
    ...
    for t in range(days):
        ...
        # Mean reversion to current level (prevents compounding fantasy)
        ...
        r_t = (mu_daily + mr_term) + sigma_t * eps + shock
        logP_t = logP_t + r_t
...
# Observed daily drift & vol
cfg = EstimationConfig()
mu_obs, sig = estimate_drift_vol(close, cfg)
ann_vol = sig * math.sqrt(252.0)
p_down = chance_of_down_day(mu_obs, sig)

# Market-adjusted drift (CAPM) + regime
mu_capm = compute_capm_mu(stock_close=close, spy_close=spy_close, rf_ann=0.03)
regime = determine_regime(spy_close)
mu_daily = blend_mu(mu_obs=mu_obs, mu_capm_daily=mu_capm, regime=regime)

# Simulate with realistic mechanics (same simple UI)
with st.spinner("Projecting forward…"):
    mean_path, pct = simulate_path_realistic(
        S0=S0, mu_daily=mu_daily, sigma0=sig, days=int(horizon), n_paths=int(n_paths), seed=int(seed),
        mean_revert_kappa=0.004, shock_prob=0.02, shock_sigma_mult=2.5
    )

# ==============================
# Top cards (unchanged style)
# ==============================
...
# ==============================
# Key numbers & more realistic verdict
# ==============================
...
# Verdict: require positive return AND some risk-adjusted quality
ret_1y = (proj_1y / S0 - 1.0) if np.isfinite(proj_1y) and S0 > 0 else np.nan
sharpe_like = (ret_1y / (ann_vol + 1e-12)) if np.isfinite(ret_1y) and np.isfinite(ann_vol) and ann_vol > 1e-12 else np.nan

if np.isfinite(ret_1y) and np.isfinite(sharpe_like):
    if ret_1y > 0.02 and sharpe_like > 0.15:
        verdict = "Favorable"
    elif ret_1y < -0.02 or sharpe_like < 0.05:
        verdict = "High risk"
    else:
        verdict = "Mixed"
else:
...
st.write(
    ...
    f"- In plain English: Considering recent behavior **and** broader market context, the stock’s middle path reaches "
